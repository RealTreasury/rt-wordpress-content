...
class TreasuryTechPortal {
    ...
    async loadTools() {
        if (this.TREASURY_TOOLS.length) return;
        const cached = localStorage.getItem('ttp_tools');
        if (cached) {
            try {
                this.TREASURY_TOOLS = JSON.parse(cached);
                return;
            } catch (e) {}
        }
        try {
            const url = (typeof TTP_DATA !== 'undefined' && TTP_DATA.rest_url) ? `${TTP_DATA.rest_url}?per_page=100&page=1` : `${EMBED_ORIGIN}/wp-json/ttp/v1/tools?per_page=100&page=1`;
            const resp = await fetch(url);
            if (resp.ok) {
                this.TREASURY_TOOLS = await resp.json();
                localStorage.setItem('ttp_tools', JSON.stringify(this.TREASURY_TOOLS));
            }
        } catch (e) {
